...
tc qdisc add dev ${OUT_INTERFACE} root handle 1 cbq bandwidth 50Mbit avpkt 1000 cell 8

#creo la classe per gestione del ping
tc class add dev ${OUT_INTERFACE} parent 1: classid 1:10 cbq bandwidth 15Mbit rate 3Kbit weight 1Kbit prio 5 allot 1514 avpkt 1000 bounded
...
tc class add dev ${OUT_INTERFACE} parent 1: classid 1:20 cbq bandwidth 15Mbit rate 100kBit weight 100Kbit prio 7 allot 1514 avpkt 1000 bounded

#creo la coda associata alla classe per connessioni tcp
tc qdisc add dev ${OUT_INTERFACE} parent 1:20 handle 20 netem delay 200ms 20ms distribution normal loss 5% duplicate 20%
...
tc class add dev ${OUT_INTERFACE} parent 1: classid 1:30 cbq bandwidth 20Mbit rate 20MBit weight 2Mbit prio 8 allot 1514 avpkt 1000 bounded

#creo la coda associata alla classe per connessioni ssh
tc qdisc add dev ${OUT_INTERFACE} parent 1:30 handle 30 netem delay 200ms 20ms distribution normal loss 5% duplicate 20%
